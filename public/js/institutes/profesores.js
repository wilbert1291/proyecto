...
$(document).ready(function () {
    tabla = $('#tabla_profesores').DataTable({
        "destroy": true,
        "ajax": "profesores/cargar_tabla",
        "columns": [{
            "data": "clave_usuario",
            "visible": false
        }, {
            ...
            "defaultContent": "<button class='btn btn-warning editar' data-toggle='modal' data-target='#modal_profesores'><i class='far fa-edit'></i></button><button class='btn btn-danger eliminar'><i class='far fa-trash-alt'></i></button>"
        }],
        "language": idioma
    });

    $('#tabla_profesores tbody').on("click", "button.editar", function () {
    ...
    $('#tabla_profesores tbody').on("click", "button.eliminar", function () {
        var data = tabla.row($(this).parents("tr")).data();
...
function cargar_municipios(id_municipio) {
    if ($('#estado').val() != 0) {
        $.ajax({
            url: "profesores/combo_municipios",
            type: 'POST',
            dataType: 'html',
            data: {
...
function cargar_localidades(id_municipio, id_localidad) {
    if ($('#municipio').val() != 0) {
        $.ajax({
            url: "profesores/combo_localidades",
            type: 'POST',
            dataType: 'html',
            data: {
                "id_municipio": id_municipio,
...
function validar(form) {
    ...
    if ($('#nombre').val() == "" || $('#ap').val() == "" || $('#am').val() == "" || $('#correo').val() == "" || $('#telefono').val() == "" || $('#curp').val() == "" || $('#calle').val() == "" || $('#colonia').val() == "" || $('#codigo_postal').val() == "" || $('#usuario').val() == "" || $('#password').val() == "" || $('#pregunta_secreta').val() == "" || $('#respuesta').val() == "") {
        swal({
            title: "Ohh ohh!",
            text: "Existen campos vacios",
            icon: "error",
